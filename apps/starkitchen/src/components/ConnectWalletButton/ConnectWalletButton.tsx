...
const colors = ['from-purple-500 to-indigo-600', 'from-blue-500 to-cyan-500', 'from-green-500 to-teal-500', 'from-red-500 to-pink-500', 'from-yellow-500 to-orange-500', 'from-gray-700 to-gray-900', 'from-indigo-500 to-purple-500', 'from-teal-500 to-green-500', 'from-orange-500 to-red-500', 'from-blue-400 to-blue-600']
...
export const ConnectWalletButton = ({onConnect}: {onConnect?: () => void}) => {
  ...
  return (
    <>
      ...
      <ConnectWalletDialog open={open} onToggleVisibility={onToggleVisibility} />
    </>
  )
}

const ConnectWalletDialog = ({open, onToggleVisibility}: { open: boolean; onToggleVisibility: () => void }) => {
  const {connectors, connectAsync} = useConnect();
  const [connectingWalletIndex, setConnectingWalletIndex] = useState<number | null>(null);

  return (
    <AlertDialog open={open} onOpenChange={onToggleVisibility}>
        <AlertDialogContent className="sm:max-w-[425px]">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-bold">Connect Wallet</AlertDialogTitle>
                ...
                >
                  <div className="flex items-center">
                    <div className="w-8 h-8 mr-3 bg-white rounded-full flex items-center justify-center">
                      <img src={typeof connector.icon === 'string' ? connector.icon : connector.icon.light} width={20} height={20} />
                    </div>
